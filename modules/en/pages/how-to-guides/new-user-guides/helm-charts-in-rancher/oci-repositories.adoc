...
If you need to update immediately, you can ../helm-charts-in-rancher/helm-charts-in-rancher.adoc#refresh-chart-repositories[perform a manual refresh].
...
* To enhance logging information, ../../../troubleshooting/other-troubleshooting-tips/logging.adoc#kubernetes-install[enable the debug option] while deploying Rancher.
...
* Apps installed through OCI-based Helm chart repositories are subject to a known issue with how Rancher displays upgradeable version information. See the ./helm-charts-in-rancher.adoc#limitations[Limitations] section of *Helm Charts and Apps* for more details.
