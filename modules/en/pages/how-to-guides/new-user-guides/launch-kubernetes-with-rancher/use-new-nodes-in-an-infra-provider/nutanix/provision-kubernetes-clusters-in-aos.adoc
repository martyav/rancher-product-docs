...
. Locate Rancher's built-in Nutanix ../../../authentication-permissions-and-global-configuration/about-provisioning-drivers/manage-node-drivers.adoc#activatingdeactivating-node-drivers[node driver and activate it].
...
Creating a ../use-new-nodes-in-an-infra-provider.adoc#node-templates[node template] for Nutanix AOS will allow Rancher to provision new nodes in Nutanix AOS. Node templates can be reused for other clusters.
