...
| Credential Secret
| Choose the credentials for S3 from your secrets in Rancher. backup-configuration.adoc#example-credentialsecret[Example].
...
Select an existing Persistent Volume (PV) that will be used to store your backups. For information about creating PersistentVolumes in Rancher, refer to ../../how-to-guides/new-user-guides/manage-clusters/create-kubernetes-persistent-storage/manage-persistent-storage/set-up-existing-storage.adoc#2-add-a-persistentvolume-that-refers-to-the-persistent-storage[this section.]
