...
If you already have an EKS Kubernetes cluster, skip to the step about <<5-install-an-ingress,installing an ingress.>> Then install the Rancher Helm chart following the instructions on install-upgrade-on-a-kubernetes-cluster.adoc#install-the-rancher-helm-chart[this page.]
