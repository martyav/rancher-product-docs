...
| `agentTLSMode`
| ""
| `string` - either `system-store` or `strict`. See ./tls-settings.adoc#agent-tls-enforcement[Agent TLS Enforcement]
|
...
When you log in for the first time, use the bootstrap password you set to log in. If you did not set a bootstrap password, the Rancher UI shows commands that can be used to ../resources/bootstrap-password.adoc#retrieving-the-bootstrap-password[retrieve the bootstrap password]. Run those commands and log in to the account. After you log in for the first time, you are asked to reset the admin password.
